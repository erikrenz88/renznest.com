...
filters:
  - social-share
share:
  permalink: "https://www.renznest.com/posts/250402-mediprogram/index.html"
  description: "Who can say goodbye to healthcare? Mostly, the ones that need it."
...
```{r}
# Official CMS/Medicaid.gov open-data source for the same figures that used
# to be scraped from the (now bot-blocked) report-highlights page. This is
# the Performance Indicator dataset: https://data.medicaid.gov/dataset/6165f45b-ca93-5bb5-9d06-db29c692a360
# The CSV filename changes with each monthly release, so resolve today's
# download URL from the dataset's metadata rather than hardcoding it.
medicaid_dataset <- jsonlite::fromJSON(
  "https://data.medicaid.gov/api/1/metastore/schemas/dataset/items/6165f45b-ca93-5bb5-9d06-db29c692a360"
)
medicaid_csv_url <- medicaid_dataset$distribution$downloadURL[1]

medicaid_raw <- read_csv(medicaid_csv_url, show_col_types = FALSE) |>
  clean_names()

# Each state reports a "Preliminary" and later "Updated"/final row per month;
# use the most recent month that has a final report for every state.
latest_final_period <- medicaid_raw |>
  filter(final_report == "Y") |>
  pull(reporting_period) |>
  max()

medicaid <- medicaid_raw |>
  filter(final_report == "Y", reporting_period == latest_final_period) |>
  select(state = state_name, medicaid_enrollment = total_medicaid_enrollment) |>
  pivot_longer(
    cols = !state,
    names_to = 'metric',
    values_to = 'value'
