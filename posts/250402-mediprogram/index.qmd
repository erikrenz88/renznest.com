...
pov <- get_acs(
  ...
  variables = c(
    ...
    between_125_149 = "B17024_006"  # We’ll take ~38% of this
  ),
  year = 2023,
  survey = "acs1",
  geometry = FALSE
...
poverty <- pov %>%
  mutate(
    medicaid_eligible_perc = sum(under_50, between_50_74, between_75_99, between_100_124, between_125_149)/pop_total,  # or a sum of multiple variables if you're combining ranges
...
population_data <- read_xlsx(here::here('_dev/250402-mediprogram/SCPRC-EST2024-18+POP.xlsx'), skip = 3) %>%
  select(`...1`, 'Number') %>%
  rename(state = `...1`) %>%
  filter(!is.na(Number)) %>%
