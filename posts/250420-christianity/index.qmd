...
```{r}
# catechism pdf url
cat_url <- 'https://media.githubusercontent.com/media/erikrenz88/renznest.com/refs/heads/main/posts/250420-christianity/Catechism_of_the_Catholic_Church.pdf'

cat_pdf <- download.file(cat_url)

# load catechsim for word analysis
cat_text <- pdftools::pdf_text(cat_pdf)
cat_all <- paste(cat_text, collapse ='\n')

words_to_find <- c('Abortion', 'Homosexual', 'Transgender')
...
word_summary <- words_df |>
  ...
  group_by(word) |>
  summarize(count = n()) |>
  mutate(frequency = count/total_words) |>
  arrange(desc(count))
...
# Make word_summary available to Observable
word_summary <- word_summary %>%
  select(word, frequency) %>%
  arrange(desc(frequency))

ojs_define(work_summary = word_summary)
```
...
// Get the word frequencies from R
...
Plot.plot({
  ...
  marks: [
    ...
    Plot.text(frequencies, {
      x: "word",
      y: "frequency",
      text: d => (d.frequency * 100).toFixed(4) + "%",
      dy: -10,
      fontSize: 12
    })
  ],
