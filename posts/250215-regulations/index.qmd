...
df_sect <- grouped_regs |>
  ...
  mutate(
    unconstitutionality_index = num_sections / num_laws_passed
  ) |>
  left_join(doge, by = 'year') |>
  pivot_longer(
    cols = c('total_bureaucratic_terms', 'avg_complexity_ratio', 'total_word_count', 'num_laws_passed', 'num_sections', 'unconstitutionality_index', 'word_count', 'regulation_count'),
    names_to = 'metric',
    values_to = 'value'
  )
...
dataFormatted = tlr.map(d => ({...d, year: new Date(d.year, 0, 1)}))
...
Plot.plot({
  ...
  marks: [
    Plot.barY(filteredData, {fx: 'year', x: 'metric', y: 'value', fill: 'metric', tip: true}),
    Plot.ruleY([0])
  ]
})
```
...
viewof agency = Inputs.checkbox(
    [...new Set(rolledUpData.map(d => d.agency))],
...
chart1 = rolledUpData.filter(d => ['total_word_count'].includes(d.metric))

chart2 = rolledUpData.filter(d => ['regulation_count', 'total_bureaucratic_terms'].includes(d.metric))

chart3 = rolledUpData.filter(d => ['avg_complexity_ratio', 'unconstitutionality_index'].includes(d.metric))

// line chart (word count by agency)
Plot.plot({
  marginLeft: 80,
  marginBottom: 40,
  marginRight: 80,
  ...
  marks: [
    Plot.line(chart1, {x: 'year', y: 'value', stroke: 'metric', tip: true}),
    Plot.ruleY([0])
  ]
})
```
...
```{ojs}
// Grouped line chart (regulation and bureaucratic term count by agency)
Plot.plot({
  marginLeft: 80,
  marginBottom: 40,
  marginRight: 80,
  ...
  y: {label: 'Regulatory Metric'},
  color: {
    domain: ["regulation_count", "total_bureaucratic_terms"],
    range: ["#7F8E71", "#CA1551"],
    legend: true,
    tickFormat: text => text.replace(/_/g, " ").replace(/^./, str => str.toUpperCase()) 
  },
...
Plot.plot({
  ...
  marks: [
    Plot.line(chart3, {x: 'year', y: 'value', stroke: 'metric', tip: true}),
    Plot.ruleY([0])
  ]
})
```
