...
```{r}
#| label: load regulation data saved from gdrive and other csv files
library(reticulate)
library(dplyr)
library(readr)
library(tidyr)
...
# download local .csv files first
rules <- read_csv(here::here('posts/250215-regulations/federal_register_rule_counts.csv'))
doge <- read_csv(here::here('posts/250215-regulations/regulation-data.csv'))

# Download regulations from gdrive
use_python(Sys.which("python3"), required=TRUE)
gdown <- import("gdown")

# Set download ID and output variable
gdrive_id <- "1Re-xRy9d3jZmWOVjvC4uwyi8UChxzUIY"
max_retries <- 3  # Number of retries if download fails
attempt <- 1
success <- FALSE

while (!success && attempt <= max_retries) {
  message(paste("Attempting to download data (Try", attempt, "of", max_retries, ")"))

  tryCatch({
    # Directly download and load into regs
    temp_file <- tempfile(fileext = ".csv")
    gdown$download(id = gdrive_id, output = temp_file, fuzzy = TRUE, quiet = FALSE)
    
    # Check if file was downloaded
    if (file.exists(temp_file) && file.info(temp_file)$size > 0) {
      regs <- read_csv(temp_file)
      success <- TRUE
      message("Download successful! Data loaded into 'regs'")
    } else {
      message("ERROR: File missing or empty, retrying...")
    }
  }, error = function(e) {
    message("Download attempt failed: ", e$message)
  })

  Sys.sleep(5)  # Pause before retrying
  attempt <- attempt + 1
}

# Stop if all attempts fail
if (!success) {
  stop("ERROR: Failed to download data after multiple attempts.")
}
```

```{r}
#| label: data transformation 1
...
for (i in 2:length(years)) {
  ...
  prev_data <- filter(melted_regs, year == prev_year)
  curr_data <- filter(melted_regs, year == curr_year)

  prev_word_count <- sum(filter(prev_data, metric == "total_word_count")$value, na.rm = TRUE)
  curr_word_count <- sum(filter(curr_data, metric == "total_word_count")$value, na.rm = TRUE)

  prev_bureaucratic <- sum(filter(prev_data, metric == "total_bureaucratic_terms")$value, na.rm = TRUE)
  curr_bureaucratic <- sum(filter(curr_data, metric == "total_bureaucratic_terms")$value, na.rm = TRUE)

  prev_complexity <- mean(filter(prev_data, metric == "avg_complexity_ratio")$value, na.rm = TRUE)
  curr_complexity <- mean(filter(curr_data, metric == "avg_complexity_ratio")$value, na.rm = TRUE)
  ...
  efficiency_score <- ifelse(word_growth != 0, (bureaucratic_change + complexity_change) / (2 * word_growth), 0)

  analysis_results[[as.character(curr_year)]] <- data.frame(
    year = curr_year,
...
df_results <- bind_rows(analysis_results)
```

```{r}
#| label: data transformation 5
...
ojs_define(yoy = df_melted)
```

```{r}
#| label: data transformation 6
# Prepare Data for Visualization
df_melted <- df_results %>%
  pivot_longer(cols = c(word_growth, bureaucratic_change, complexity_change, efficiency_score),
...
Plot.plot({
  ...
  marks: [
    Plot.line(fyoy, {x: 'year', y: 'value', stroke: 'metric', tip: true}),
    Plot.ruleY([0])
  ]
})
```
