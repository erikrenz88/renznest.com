...
rolledUpData = Array.from(
  d3.rollup(
    ...
    d => d.year,    // "2021-01-01" (string) now
    d => d.metric
  ),
  ([year, metricMap]) =>
    Array.from(metricMap, ([metric, value]) => ({
      year: new Date(year),     // Parse from ISO-safe string
      title,                    // assumes `title` is scoped in
      metric,
      value
    }))
).flat()
  // Optional: sort by year then metric
  .sort((a, b) => a.year - b.year || a.metric.localeCompare(b.metric));

chart1 = dataFormatted.filter(d => ['total_word_count', 'DOGE_word_count'].includes(d.metric))
...
Plot.plot({
  ...
  marks: [
    Plot.barY(chart1, {fx: 'year', x: 'metric', y: 'value', fill: 'metric', z: 'metric', tip: true,
      title: d => {
        const names = {
...
rolledUp = Array.from(
  d3.rollup(
    dataFormatted,
    v => d3.sum(v, d => d.value),
    d => d.year,     // Now a string like "2021-01-01"
    d => d.title,
    d => d.metric
  ),
  ([year, titles]) =>
    Array.from(titles, ([title, metrics]) =>
      Array.from(metrics, ([metric, value]) => ({
        year: new Date(year),  // Safe parse from ISO string
        title,
        metric,
        value
      }))
  ...
  // Sort to ensure line charts behave smoothly
  .sort((a, b) => a.year - b.year || a.title.localeCompare(b.title) || a.metric.localeCompare(b.metric));

// get unique Agencies
uniqueAgencies = [...new Set(rolledUp.map(d => d.title))];
...
filtered = rolledUp.filter(d => 
  agency.includes('All') || agency.includes(d.title)
)

words = filtered.filter(d => ['total_word_count'].includes(d.metric))
...
Plot.plot({
  ...
  marks: [
    Plot.line(words, {x: 'year', y: 'value', stroke: 'metric', tip: true,
      title: d => {
        const names = {
          total_word_count: 'Word count'
        }
