...
```{python echo: true, warning: true, error: true}
#| label: download data
print("🧭 Working directory:", os.getcwd())
print("📁 Directory contents:")
...
rules_path = Path("posts/250215-regulations/federal_register_rule_counts.csv")
doge_path = Path("posts/250215-regulations/regulation-data.csv")

print("Checking rules path exists:", rules_path.exists())
print("Checking doge path exists:", doge_path.exists())
...
try:
  ...
  if doge_path.exists():
      doge = pl.read_csv(doge_path)
      # Google Drive file ID
      gdrive_id = "1Re-xRy9d3jZmWOVjvC4uwyi8UChxzUIY"
      url = f"https://drive.google.com/uc?id={gdrive_id}"

      # Download and load directly into a dataframe
      print("Downloading file into memory...")
      temp_file = gdown.download(url, quiet=False)
      regs = pl.read_csv(temp_file)
  else:
      print(f"DOGE file not found at {doge_path}")
except Exception as e:
    import traceback
    traceback.print_exc()  # 👈 prints full error with context
    raise e  # Re-raise so GitHub still fails the build
...
try:
  ...
  print("\n❌ ERROR DURING REGULATORY TRANSFORMATION BLOCK\n")
  ...
  # Show whether key files exist
  print("\n🔎 Checking file paths:")
  print("  - federal_register_rule_counts.csv:", Path("posts/250215-regulations/federal_register_rule_counts.csv").exists())
  print("  - regulation-data.csv:", Path("posts/250215-regulations/regulation-data.csv").exists())
...
# Load DOGE Data (if available)
doge_data_path = "posts/250215-regulations/regulation-data.csv"

if os.path.exists(doge_data_path):
    doge = pl.read_csv(doge_data_path)
else:
...
df_sect = (
    grouped_regs
    .join(laws_by_year_df, on="year", how="left")  # Join Laws Passed Per Year
    .join(rules, on="year", how="left")  # Join Federal Register Rules Data
    .with_columns(
        (pl.col("rule_count") / pl.col("num_laws_passed")).alias("unconstitutionality_index")
    )
    .join(doge, on="year", how="left")  # 🐶 Join DOGE Data
    .rename({"word_count": "DOGE_word_count"})  # Rename DOGE word count column
    .melt(
        id_vars=["agency", "year"],  # Keep these columns fixed
        value_vars=[
