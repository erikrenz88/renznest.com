...
for i in range(1, len(years)):
    ...
    prev_data = grouped_regs.filter(pl.col("year") == prev_year)
    curr_data = grouped_regs.filter(pl.col("year") == curr_year)
...
grouped_regs = grouped_regs.melt(
    id_vars=["title", "year"],
...
grouped_regs_json = grouped_regs.to_dicts()
yoy_json = df_results.to_dicts()

# Send to OJS
ojs_define(data=df_melted_json)
ojs_define(int=grouped_regs_json)
