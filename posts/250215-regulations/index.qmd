...
# Define file paths
rules_path = "posts/250215-regulations/federal_register_rule_counts.csv"
doge_path = "posts/250215-regulations/regulation-data.csv"
...
if os.path.exists(doge_path):
    ...
    print("DOGE data read successfully")
else:
    print(f"Warning: {doge_path} not found.")
...
regs = (
    regs.with_columns([
        ...
        pl.col("text").map_elements(lambda x: count_terms(x, bureaucratic_terms, return_dtype=pl.Int32)).alias("bureaucratic_terms"),
        pl.col("text").map_elements(lambda x: count_terms(x, explanatory_terms), return_dtype=pl.Int32).alias("explanatory_terms"),
        pl.col("text").map_elements(lambda x: len(x.split()) if x else 0, return_dtype=pl.Int32).alias("reg_word_count")  # Count words
    ])
    .with_columns([
...
df_sect = (
    grouped_regs
    .join(laws_by_year_df, on="year", how="left")  # 🏛️ Join Laws Passed Per Year
