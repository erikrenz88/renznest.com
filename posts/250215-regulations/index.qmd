...
rules_path = Path('/home/runner/work/renznest.com/renznest.com/posts/200215-regulations/federal_register_rule_counts.csv')
doge_path = Path('/home/runner/work/renznest.com/renznest.com/posts/250214-regulations/regulation-data.csv')
output = 'data.csv'
...
if doge_path.exists():
    ...
    if not Path(output).exists():
        print("📥 Downloading large regulations dataset from Google Drive...")
        gdown.download(id="1Re-xRy9d3jZmWOVjvC4uwyi8UChxzUIY", output=output, quiet=False, fuzzy=True)

    if Path(output).exists() and Path(output).stat().st_size > 0:
        regs = pl.read_csv(output)
    else:
        raise FileNotFoundError("❌ Downloaded file is missing or empty!")

else:
    print(f"⚠️ DOGE file not found at {doge_path}")
```
